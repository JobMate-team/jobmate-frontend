...
interface QuestionManagementProps {
    ...
    onAddQuestion: (question: string, category: string) => void;
    editingQuestion?: Question;
    onUpdateQuestion?: (question: string, category: string) => void;
    onCancelEdit?: () => void;
    className?: string;
}
...
const QuestionManagement = ({
    ...
    useEffect(() => {
        if (editingQuestion) {
            ...
            setIsAdding(false); // Ensure we are not in simple add mode
        }
    }, [editingQuestion]);
    ...
    return (
        <div className={`bg-white p-4 rounded-lg border border-gray-100 flex flex-col gap-4 ${className}`}>
            {showForm ? (
                <>
                ...
                <div className="flex flex-col flex-1 gap-6">
                    ...
                    <Button
                        type='button'
                        className={`w-full py-2.5 text-sm font-medium flex items-center justify-center gap-1 mt-auto ${isAddDisabled
