...
const StatsOverview = ({ coaching, newUsers, avgAnswerLength }: StatsOverviewProps) => {
    const renderGrowth = (growth: string) => {
        ...
        return (
            <div className={`flex items-center text-sm font-medium gap-1 ${isPositive ? 'text-green-500' : 'text-red-500'}`}>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            ...
            <div className='bg-white p-6 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-between h-[140px]'>
                ...
                <div className='flex flex-col gap-2'>
                    <p className='text-3xl font-semibold text-gray-900'>{Math.round(parseFloat(avgAnswerLength))}자</p>
