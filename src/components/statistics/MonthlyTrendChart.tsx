...
const MonthlyTrendChart = ({ monthlyTrend }: MonthlyTrendChartProps) => {
    ...
    const getX = (index: number) => padding.left + (index * graphWidth) / (Math.max(trendData.length - 1, 1)); // Prevent div by 0
    const getY = (value: number) => height - padding.bottom - (value / MAX_Y) * graphHeight;

    // 그리드 라인
    const yTicks = [0, Math.round(MAX_Y * 0.25), Math.round(MAX_Y * 0.5), Math.round(MAX_Y * 0.75), Math.ceil(MAX_Y)];
                ...
                <div className='min-w-[600px] relative'>
                    ...
                    {hoveredPoint && (
                        ...
                        >
                            ...
                            <p>{hoveredPoint.label}: {hoveredPoint.value}회</p>
                        </div>
                    )}
                </div>
            </div>
