...
const UserDistributionChart = ({ jobCategoryDist }: UserDistributionChartProps) => {
    ...
    // 파이 차트 슬라이스 색상
    const COLORS = ['#3B82F6', '#8B5CF6', '#EC4899', '#F59E0B', '#10B981', '#6B7280', '#EF4444', '#6366F1'];
                ...
                <div className='w-full max-w-[300px] aspect-square relative group'>
                    ...
                    {hoveredSlice && (
                        ...
                        >
                            ...
                            <p>{hoveredSlice.count}명 ({Math.round(hoveredSlice.percent)}%)</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className='grid grid-cols-4 gap-x-4 gap-y-2 mt-4 w-full px-2'>
                {slices.map((slice, i) => (
                    <div key={i} className='flex items-center gap-1.5'>
                        ...
                        <span className='text-xs text-gray-600 font-medium whitespace-nowrap' style={{ color: slice.color }}>
